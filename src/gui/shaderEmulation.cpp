...
using nytl::read;
const ShaderEmulation::OurImage ShaderEmulation::emptyImage {};
const spvm_sampler ShaderEmulation::defaultSampler {};

ShaderEmulation::~ShaderEmulation() {
	if(spvm_.context) {
		spvm_context_deinitialize(spvm_.context);
	}
}

void ShaderEmulation::init() {
	spvm_.context = spvm_context_initialize();
}

void ShaderEmulation::select() {
	dlg_assert(shaderDebugger().compiled());
	auto& compiled = *shaderDebugger().compiled();

	static_assert(sizeof(spvm_word) == sizeof(u32));
	auto ptr = reinterpret_cast<const spvm_word*>(compiled.get_ir().spirv.data());
	spvm_.program = spvm_program_create(spvm_.context, ptr, compiled.get_ir().spirv.size());
	initState();
}

void ShaderEmulation::unselect() {
	currLine_ = {};
	currFileName_ = {};
...
void ShaderEmulation::initState() {
	...
	settings.load_variable = [](struct spvm_state* state, unsigned varID,
			...
			spvm_word typeID) {
		auto* self = static_cast<ShaderEmulation*>(state->user_data);
		self->loadVar(varID,
	...
	auto defaultEntryPoint = shaderDebugger().compiled()->get_ir().default_entry_point;
	for(auto i = 0; i < spvm_.program->entry_point_count; ++i) {
		if(u32(spvm_.program->entry_points[i].id) == defaultEntryPoint) {
...
bool ShaderEmulation::stepOpcode() {
	...
	for(auto& bp : shaderDebugger().breakpoints()) {
...
void ShaderEmulation::writeImage(spvm_image&, int x, int y, int z, int layer, int level,
...
spvm_value_type ShaderEmulation::valueType(const spvm_member& member) {
	auto* resType = spvm_state_get_type_info(spvm_.state->results, &spvm_.state->results[member.type]);
	dlg_assert(resType);
	return resType->value_type;
}

void ShaderEmulation::display(const char* name, const spvm_member& member) {
	ImGui::TableNextRow();
...
std::string ShaderEmulation::formatScalar(const spvm_member& member) {
	std::string str;
...
void ShaderEmulation::loadBuiltin(const spc::BuiltInResource& builtin,
	...
	auto wgs = shaderDebugger().workgroupSize();
	auto numWGs = shaderDebugger().numWorkgroups();
...
unsigned ShaderEmulation::arrayLength(unsigned varID, span<const spvm_word> indices) {
	ZoneScoped;
	dlg_assert(shaderDebugger().compiled());
	auto& compiled = *shaderDebugger().compiled();

	auto res = resource(compiled, varID);
	if(!res) {
	...
	auto& selection = shaderDebugger().selection();
...
std::pair<const Type*, u32> ShaderEmulation::accessBuffer(ThreadMemScope& tms,
		unsigned typeID, span<const spvm_word> indices, u32 dataSize) {
	const auto* type = buildType(*shaderDebugger().compiled(), typeID, tms.customUse());
	dlg_assert(type);
...
void ShaderEmulation::loadVar(unsigned srcID, span<const spvm_word> indices,
		span<spvm_member> dst, u32 typeID) {
	ZoneScoped;
	dlg_assert(shaderDebugger().compiled());
	auto& compiled = *shaderDebugger().compiled();

	auto res = resource(compiled, srcID);
	if(!res) {
		auto builtin = builtinResource(compiled, srcID);
		if(!builtin) {
	...
	auto* baseCmd = shaderDebugger().selection().command().back();
	auto* cmd = deriveCast<const StateCmdBase*>(baseCmd);
	auto& dsState = shaderDebugger().selection().descriptorSnapshot();
	auto dss = cmd->boundDescriptors().descriptorSets;
	...
	auto hookState = shaderDebugger().selection().completedHookState();
	dlg_assert(hookState);
	// dsCopyIt->second stores the beginning of the range we stored in the
	// array elements in. So we have to offset it with the arrayElemID
	dlg_assert(hookState->copiedDescriptors.size() > dsCopyIt->second + arrayElemID);
...
void ShaderEmulation::storeVar(unsigned id, span<const spvm_word> indices,
...
void ShaderEmulation::updateHooks(CommandHook& hook) {
	dlg_assert(shaderDebugger().compiled());
	auto& compiled = *shaderDebugger().compiled();
	...
	shaderDebugger().selection().clearState();
}

void ShaderEmulation::initVarMap() {
	dlg_assert(shaderDebugger().selection().completedHookState());
	auto& state = *shaderDebugger().selection().completedHookState();
...
void ShaderEmulation::setupScalar(const Type& type, ReadBuf data, spvm_member& dst) {
...
void ShaderEmulation::setupVector(const Type& type, u32 stride, ReadBuf data,
		spvm_member& dst) {
...
void ShaderEmulation::setupMember(const Type& type, ReadBuf data, spvm_member& dst) {
	if(!type.array.empty()) {
...
void ShaderEmulation::setupMemberArray(span<const u32> arrayDims,
...
spvm_sampler_desc ShaderEmulation::setupSampler(const Sampler& src) {
...
spvm_vec4f ShaderEmulation::readImage(spvm_image& srcImg, int x, int y, int z, int layer, int level) {
...
void ShaderEmulation::drawControls() {
	// TODO
	/*
	dlg_assert(spvm_.state);

	auto isLastReturn = [this](){ // TODO: kinda hacky. I guess spvm should have a function
...
void ShaderEmulation::updateFromEditor() {
...
/*
void ShaderEmulation::drawVariablesTab() {
...
void ShaderEmulation::drawBreakpointsTab() {
...
void ShaderEmulation::drawCallstackTab() {
...
void ShaderEmulation::drawControlTabs() {
	/*
