...
Gui::Gui(Device& dev, VkFormat colorFormat) {
	...
	tabs_.events = std::make_unique<EventViewer>();
...
void Gui::drawEventUI(Draw&) {
	...
	if(ImGui::BeginTable("Events", cols, flags)) {
		...
		for(auto& event : dev_->eventLog->events) {
			...
			if(ImGui::TreeNode(event.get(), "%s", name(event->type))) {
				...
				if(event->type == EventType::queueSubmit) {
					auto& qse = static_cast<QueueSubmitEvent&>(*event);
					refButtonExpect(*this, qse.submission.queue);
					// TODO: allow to select submission
				}
